:page-role: deprecated-5.26 not-on-aura
...
[NOTE]
====
The transactional HTTP API is deprecated and replaced by the HTTP Query API.
See link:{neo4j-docs-base-uri}/query-api/current/plain-json/[Query API -> Plain JSON] and link:{neo4j-docs-base-uri}/query-api/current/typed-json/[Query API -> Typed JSON] for details on how to receive query results in different formats with the Query API.
====
