...
|label:deprecated[Deprecated in 5.26] To submit queries (in an implicit transaction). +
For more information, see link:{neo4j-docs-base-uri}/query-api/current/endpoints/[Query API -> Endpoints].

|/db/<databaseName>/tx
|label:deprecated[Deprecated in 5.26] To open a transaction. +
For more information, see link:{neo4j-docs-base-uri}/query-api/current/endpoints/[Query API -> Endpoints].

|/db/<databaseName>/tx/<transactionID>
|label:deprecated[Deprecated in 5.26] To submit queries to an open transaction with ID `<transactionID>`. +
A `DELETE` request results in a rollback. +
For more information, see link:{neo4j-docs-base-uri}/query-api/current/endpoints/[Query API -> Endpoints].

|/db/<databaseName>/tx/<transactionID>/commit
|label:deprecated[Deprecated in 5.26] To commit an open transaction with ID `<transactionID>`. +
For more information, see link:{neo4j-docs-base-uri}/query-api/current/endpoints/[Query API -> Endpoints].
